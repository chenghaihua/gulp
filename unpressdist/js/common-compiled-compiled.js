    ...
    $.get(hostUrl + "sale_list", { pg: pg, gh: gh, gg: gg }, function (data) {
        if (data.status) {
            ...
            for (var i = 0, iLen = dataMes.length; i < iLen; i++) {
                ...
                str3 += '<li data-sale_id="' + dataMes[i].id + '">' + '<div>' + '<span>' + dataMes[i].gh + '</span>' + '<span>' + dataMes[i].gg + '</span>' + '<span>' + dataMes[i].gc + '</span>' + '<span>' + dataMes[i].cd + '米</span>' + '<span class="sale_num">' + dataMes[i].sl + '吨</span>' + '</div>' + '<div>' + '<span>' + dataMes[i].price + '元/吨</span>' + '<a href="javascript:void(0);" data-itrue="' + it + '" class=' + active2 + '>加入采购清单</a>' + '</div>' + '</li>';
            }
            //console.log(str3);
